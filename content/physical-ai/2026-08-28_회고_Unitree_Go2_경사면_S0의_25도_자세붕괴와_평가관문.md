---
title: "Unitree Go2 R0 depenetration 속도의 접촉력·침투 상충"
...
tags:
  ...
  - PPO
  - 사족보행
description: "25도 자세 붕괴와 R0 Gate10 접촉을 귀속하고, depenetration 속도를 낮춘 rev14가 힘은 줄였지만 침투 깊이에서 기각된 과정을 정리했어요."
publish: true
section: physical-ai
---
...
다음 rev15는 기각된 rev13·rev14 설정을 이어받지 않아요. 마지막 runtime 승인본인 rev12의 solver `8/0`과 `max_depenetration_velocity=1.0m/s`로 돌아가 position iteration만 `8 → 16`으로 바꾸는 새 scratch 단일변수 실험이에요. position iteration은 관절과 접촉 constraint를 한 physics step 안에서 더 수렴시키는 방향이지만 계산 비용도 늘어요. contact offset과 rest offset은 충돌 시작 거리와 정지 간격까지 바꾸므로 이번 revision에서는 건드리지 않아요.

rev15는 `CPU 3회 → GPU 3회 → strict synthesis → scratch Gate01` 순서로 검사해요. CPU 단계에서 separation, force, root height, numeric-invalid와 hard-limit 중 하나라도 실패하면 GPU를 열지 않아요. 이 경로를 통과해야 Gate10을 실행하고, 이후 `1,024 env × 24 step × 300 iterations` qualification에서 네 전복 자세 각각 성공률 `80% 이상`, 중앙 복구시간 `4초 이하`, safety termination `0`을 요구해요.
...
25도에서 확인한 자세 붕괴는 로봇의 최대 경사를 확정한 결과가 아니에요. R0 Gate10의 세 hard-limit 사건도 전복 복구 성능이 생겼다는 결과가 아니에요. rev13과 rev14는 실제 설정 readback이 맞더라도 force와 separation을 함께 통과하지 못하면 학습을 시작하지 않는 절차를 남겼어요. rev15도 한 물리 변수만 바꾸고 CPU 결과부터 판정해요.

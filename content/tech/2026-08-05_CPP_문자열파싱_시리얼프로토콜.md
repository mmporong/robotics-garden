...
view는 소유하지 않으니 원본이 사라지는 순간 해제된 메모리를 가리켜요. [[2026-08-05_CPP_소유권과_수명_스마트포인터|C++ 소유권과 수명 — 스마트 포인터]]의 참조 캡처 댕글링과 정확히 같은 뿌리예요. 가장 잘 나는 자리가 임시 string이에요.
...
이 파서 앞에는 프레이밍 단계가 하나 더 있어요. 수신 버퍼에 계속 덧붙이고 개행이 보일 때만 한 줄을 떼어내는 구조인데, [[2026-08-05_CPP_시리얼IO_termios_프레임재조립|C++ 시리얼 IO — termios와 프레임 재조립]]에 자세히 있어요.

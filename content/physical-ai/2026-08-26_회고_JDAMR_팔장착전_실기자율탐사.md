...
이번에 확인한 범위는 Cartographer 지도 생성과 frontier 자율탐사의 종료·저장이에요. 저장 지도를 다시 불러온 localization 재현성까지 확인한 결과는 아니에요. 다음 실기에서는 아래 항목을 같은 시나리오로 기록할 계획이에요.

- AMCL 초기 자세가 수렴하는 시간과 최종 오차
- 같은 Nav2 목표를 반복했을 때의 성공·취소·복구 횟수
- 위치를 일부러 틀어 준 뒤 재지역화되는 시간
- recovery와 Collision Monitor 개입 시점, 당시 costmap·TF 상태

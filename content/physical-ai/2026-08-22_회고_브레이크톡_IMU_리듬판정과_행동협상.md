...
Toython 2026(서울 AI 허브, NUCODE NU-40 DK 필수)에 개인으로 나가 브레이크톡을 냈어요. 숏폼을 줄이고 싶은 사람을 위한 기기로, 스마트폰 뒷면에 자석으로 붙이고 화면 권한 없이 6축 IMU만으로 손의 리듬을 읽어요. 빠르게 넘기는 구간을 잡으면 짧은 메시지를 보내고, 그 뒤 20초 안에 멈췄는지 계속 넘겼는지를 답으로 기록해서 텔레그램 푸시와 라이브 화면, 주간 리포트로 돌려줘요. 48팀 중 1차 심사 3위로 본선 20팀에 올라갔고 최종 수상은 아니었어요.

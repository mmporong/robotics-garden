---
title: "Unitree Go2 R0 접촉 귀속과 solver velocity 기각"
...
tags:
  ...
  - PPO
  - 사족보행
description: "25도 자세 붕괴와 R0 Gate10 접촉을 귀속하고, solver velocity A/B가 CPU 안전 관문에서 기각된 이유와 다음 단일변수 검증을 정리했어요."
publish: true
section: physical-ai
---
...
다음 rev14는 solver `8/1`을 유지하고 rigid body의 `max_depenetration_velocity`만 `1.0 → 0.75m/s`로 낮춰요. 이 값은 solver가 접촉 침투를 풀려고 도입할 수 있는 속도의 상한이에요. 먼저 모든 rigid link의 live readback을 확인한 뒤 같은 CPU probe를 세 번 실행해요. 세 실행 모두 오른쪽 옆면 base 접촉 peak가 rev12 기준 `9.332861 BW` 이하이고 numeric-invalid와 hard-limit termination이 0이며 `root_height_above_2cm` 검사가 true일 때만 GPU runtime을 열어요. `max_contact_impulse`는 peak를 직접 잘라 실패를 숨길 수 있어 이번 실험에서는 쓰지 않아요.

CPU 관문을 통과하더라도 곧바로 성공으로 판정하지 않아요. `GPU runtime 3회 → scratch Gate01 → scratch Gate10`을 순서대로 통과한 revision만 `1,024 env × 24 step × 300 iterations` qualification으로 넘겨요. 최종 checkpoint는 네 전복 자세 각각 성공률 `80% 이상`, 중앙 복구시간 `4초 이하`, safety termination `0`을 모두 만족해야 승인해요.
...
- [G009 전체 기술 문서](https://github.com/mmporong/isaac-walk-rl/blob/9392f17dd29748bcf74af3ab4d341eb56f898ce6/docs/G009_MOUNTAIN_SLOPE_RECOVERY.md)
...
25도에서 확인한 자세 붕괴는 로봇의 최대 경사를 확정한 결과가 아니에요. R0 Gate10의 세 hard-limit 사건도 전복 복구 성능이 생겼다는 결과가 아니에요. rev13은 실제 설정 readback이 맞더라도 접촉 관문을 넘지 못하면 학습을 시작하지 않는 절차를 보여줬어요. rev14도 같은 기준으로 한 물리 변수만 바꾸고 CPU 결과부터 판정해요.

---
title: "Unitree Go2 R0의 CPU·GPU 접촉 집중도 진단"
...
tags:
  ...
  - PPO
  - 사족보행
description: "25도 자세 붕괴와 R0 Gate10 접촉을 귀속하고, rev16의 CPU·GPU 12회 진단에서 접촉 집중도 가설을 지지하지 못해 PPO 전에 멈춘 과정을 정리했어요."
publish: true
section: physical-ai
---
...
앞선 [[2026-08-27_회고_Unitree_Go2_불규칙도로_PPO_우회전보상실패|불규칙 도로 PPO 실험]]에서도 움직이는 장면보다 방향별 blocking cell이 정책 채택을 결정했어요. G009는 같은 원칙을 경사·전복 복구의 물리 안전 관문까지 확장한 작업이에요.
...
<figure class="fig"><img src="../assets/2026-08-28_g009_r0_rev13_cpu_runtime_failure.gif" alt="rev12와 rev13 CPU runtime 접촉 peak를 비교하는 진단 텔레메트리 GIF"><figcaption>세 번의 CPU 실행에서 반복된 접촉 허용 기준 초과를 비교했어요. 카메라 영상이나 PPO 학습 영상이 아니라 실행 report를 그린 진단 텔레메트리예요.</figcaption></figure>
...
rev14는 rev13의 articulation solver `8/1`을 유지하고 rigid body의 `max_depenetration_velocity`만 `1.0 → 0.75m/s`로 낮췄어요. 이 값은 solver가 겹친 물체를 분리하려고 넣을 수 있는 보정 속도의 최대값이에요. 이 값을 낮추면 짧은 접촉 반응이 완만해질 수 있지만, 같은 시간 안에 침투를 충분히 해소하지 못할 수도 있어요. force와 penetration을 따로 검사한 이유예요.
...
## rev16은 divergence 구간을 좁혔지만 가설을 지지하지 못했어요

rev16은 새 PPO revision이 아니라 CPU/GPU backend divergence를 귀속하는 진단 protocol이에요. 가설은 position iteration 16이 GPU의 right-side base 접촉 impulse를 CPU와 rev12의 position iteration 8보다 더 이르고 좁게 집중시키는지였어요. 실행 전에 다음 순서와 중단 기준을 고정했어요.

1. Arm A는 승인된 rev12 solver `8/0`을 CPU 3회, GPU 3회로 재생해 새 계측기가 기준 동역학을 바꾸지 않았는지 확인해요.
2. Arm B는 position iteration만 `8 → 16`으로 바꾸고 CPU 3회 뒤 GPU 3회를 실행해요.
3. 각 실행은 seed `42`, headless, 8개 환경, 600 physics row와 150 control row로 고정해요. physics/control timestep은 `0.005/0.02초`예요.
4. peak 전후 창에서 `peak impulse / window impulse`를 접촉 집중도로 계산하고 force peak 시각, action·EMA trace, root angular speed와 joint speed를 함께 비교해요.

실행 코드는 `9ac874f48a1403e0ed838beb5e75938db5873d1c`, 실행 소스 묶음의 SHA-256은 `8b4031ad519a7487aff4eda83638c571d6494524b8872f229eba11fdb618541a`로 고정했어요. 역사적 수치를 재현하는 native Torch float32 projection과 현재 canonical 계산을 별도 경로로 나눴어요. 역사적 비교의 절대 허용 오차 `1e-6`은 그대로 두고, 두 projection 사이에는 최대 `4e-6`과 `15 BW` 기준의 동일 판정을 함께 요구했어요. 실제 최대 차이는 `2.3343854494e-6`이었고 12회 모두 두 검사를 통과했어요.

| 실행군 | solver | backend | right-side base peak | physics step | 접촉 집중도 |
| :--- | :---: | :---: | ---: | ---: | ---: |
| Arm A | `8/0` | CPU | `9.3328602041 BW` | `131` | `0.4887608254` |
| Arm A | `8/0` | GPU | `8.7950077539 BW` | `130` | `0.4543198511` |
| Arm B | `16/0` | CPU | `13.2482805877 BW` | `130` | `0.6737326952` |
| Arm B | `16/0` | GPU | `16.7882770994 BW` | `129` | `0.7974004593` |

Arm B GPU는 세 번 모두 `15 BW`를 넘었고 peak도 Arm B CPU보다 한 physics step 일렀어요. 집중도는 Arm A GPU보다 컸고 action·EMA trace 오차는 `0`, root·joint speed 상승과 numeric-invalid·hard-joint-limit `0`도 확인했어요. 그러나 Arm B GPU와 CPU의 집중도 비율은 세 번 모두 `1.183556126964255`였어요. 사전에 고정한 `1.20`에 미치지 못한 유일한 실패 검사예요. 기준을 결과 뒤에 낮추거나 나머지 검사와 평균내지 않았어요. 최종 판정은 `hypothesis=inconclusive`, `supported_3_of_3=false`예요.

GPU의 contact pair와 separation은 사용할 수 없어 해당 필드를 권위 있는 판정에 넣지 않았어요. 이 두 항목은 CPU 결과만 authority로 취급했어요. position iteration 16은 rev15 판정대로 기각 상태를 유지해요. 이번 결과는 학습된 정책이 아니며 `learned=false`, Gate01·Gate10은 `forbidden`, qualification은 `not_run`이에요. reward 계약은 바꾸지 않았지만 PPO에 사용하지 않았고 rollout batch, mini-batch, epoch, optimizer update는 모두 `0`이에요.

<figure class="fig"><img src="../assets/2026-08-28_g009_r0_rev16_08_b_gpu_right_side_force_repro.gif" alt="rev16 Arm B GPU 오른쪽 옆면 force 재현 조건의 실제 Isaac Sim 카메라 GIF"><figcaption>번호 <code>08</code>은 Arm B solver <code>16/0</code>을 <code>cuda:0</code>에서 실행한 <code>right_side / reset_pose_hold</code> 조건의 실제 Isaac Sim headless off-screen camera footage예요. 조건이 일치하는 시각 재생이며 원 report의 힘을 픽셀에서 측정한 자료는 아니에요.</figcaption></figure>

<figure class="fig"><img src="../assets/2026-08-28_g009_r0_rev16_08_b_gpu_right_side_force_repro.png" alt="rev16 Arm B GPU 오른쪽 옆면 실제 카메라 대표 프레임"><figcaption>번호 <code>08</code>의 대표 프레임이에요. 진단 조건, <code>NO PPO</code>, <code>NOT QUALIFIED</code>, <code>REJECTED</code>를 화면에 남겨 복구 정책 영상과 구분했어요.</figcaption></figure>

<figure class="fig"><img src="../assets/2026-08-28_g009_r0_rev16_09_four_group_telemetry.gif" alt="rev16 Arm A와 Arm B의 CPU GPU 네 실행군을 비교하는 텔레메트리 애니메이션"><figcaption>번호 <code>09</code>는 12개 report의 네 실행군을 비교한 telemetry animation이에요. 실제 카메라 영상이 아니며, 유일한 실패 검사인 집중도 비율 <code>1.183556 &lt; 1.20</code>을 포함해요.</figcaption></figure>

<figure class="fig"><img src="../assets/2026-08-28_g009_r0_rev16_09_four_group_telemetry.png" alt="rev16 네 실행군의 force peak 시각과 접촉 집중도 판정 차트"><figcaption>번호 <code>09</code>의 정적 차트예요. force와 peak 시각은 가설의 일부를 지지했지만 집중도 증가 기준을 넘지 못해 전체 판정은 <code>inconclusive</code>예요.</figcaption></figure>

원본 H.264 MP4는 `%USERPROFILE%\IsaacLab\logs\visual_evidence\g009\R0\diagnostic\g009_5_r0_diag_rev16_08_b_gpu_right_side_force_repro_s42.mp4`에만 보관해요. 크기는 `267,188바이트`, SHA-256은 `151146e078ce19f113e197fef931c4e32014424af2d7ce0ef20db7f6c40618b0`이에요. 실행 데이터는 커밋 `9ac874f48a1403e0ed838beb5e75938db5873d1c`와 source bundle `8b4031ad519a7487aff4eda83638c571d6494524b8872f229eba11fdb618541a`, 실제 카메라 recorder는 clean capture 커밋 `51f2c63eaf408525fc5ddce3249f8138b8c5baaa`와 capture bundle `599487d4669b90472688428b2c9feb6f1d527235eec4e7017f0f2f2edd9962e1`로 나눠 기록했어요. Git에는 GIF·PNG·JSON만 올려요.

다음에는 Arm B CPU와 GPU의 physics step `128~130`에서 impulse 분자·분모를 나누고 base와 각 link로 이어지는 하중 경로, contact topology를 비교해요. 그 차이가 설명된 뒤에만 승인된 rev12 `8/0`에서 물리 변수 하나를 바꾼 scratch 후보를 설계해요. rev16 결과만으로 PPO를 바로 시작하지 않아요.
...
- [rev16 12회 최종 synthesis](https://github.com/mmporong/isaac-walk-rl/blob/main/reports/runs/g009_r0_rev16_synthesis_12_full_retry01_s42.json)
- [rev16 08 실제 CUDA 카메라 capture sidecar](https://github.com/mmporong/isaac-walk-rl/blob/main/reports/runs/g009_5_r0_diag_rev16_08_b_gpu_right_side_force_repro_capture_s42.json)
- [rev16 08 카메라 공개 파생물 sidecar](https://github.com/mmporong/isaac-walk-rl/blob/main/reports/runs/g009_5_r0_diag_rev16_08_b_gpu_right_side_force_repro_visual_evidence.json)
- [rev16 09 텔레메트리 공개 파생물 sidecar](https://github.com/mmporong/isaac-walk-rl/blob/main/reports/runs/g009_5_r0_diag_rev16_09_four_group_telemetry_visual_evidence.json)
...
25도에서 확인한 자세 붕괴는 로봇의 최대 경사를 확정한 결과가 아니에요. R0 Gate10의 세 hard-limit 사건도 전복 복구 성능이 생겼다는 결과가 아니에요. rev13과 rev14는 실제 설정 readback이 맞더라도 force와 separation을 함께 통과하지 못하면 학습을 시작하지 않는 절차를 남겼어요. rev15는 CPU를 통과해도 GPU가 같은 결과를 내지 않으면 멈추는 관문을 추가했어요. rev16은 차이가 커지는 구간을 physics step `128~130`으로 좁혔지만 접촉 집중도가 사전 기준에 미치지 못해 가설을 지지하지 않았어요. 다음에는 이 구간의 impulse와 하중 경로를 분해하고, 원인이 설명된 뒤에만 rev12 `8/0`에서 한 변수 scratch 후보를 설계해요.

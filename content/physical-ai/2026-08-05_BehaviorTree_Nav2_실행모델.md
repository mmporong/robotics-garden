...
경로 추종은 수십 초 걸리는 일이에요. 만약 추종 액션이 틱 안에서 도착할 때까지 기다렸다가 성공을 반환한다면 **그동안 트리 전체가 멈춰요.** 조건 재평가도, 우선순위 높은 가지로의 전환도, 복구도 전부 죽습니다. [[2026-08-05_CPP_rclcpp_노드작성_실전|rclcpp]]의 "콜백 안에서 기다리지 않는다"와 같은 원리예요. 틱은 모두가 공유하는 실행 흐름이라 한 노드가 붙잡고 있으면 안 돼요.
...
그리고 이 트리는 코드가 아니라 파라미터예요. 다른 XML 파일을 지정하면 재컴파일 없이 시나리오가 통째로 바뀌고, 목표마다 다른 트리를 쓸 수도 있어요. XML의 노드 이름 하나하나가 로드되는 플러그인이라, [[2026-08-05_CPP_인터페이스_플러그인_로드원리|인터페이스와 플러그인]]의 구조가 여기서도 반복됩니다.
...
트리가 부리는 플러그인 중 경로를 만드는 쪽은 [[2026-08-05_경로계획_다익스트라와_Astar|경로 계획]]에, 그 경로를 따라가는 쪽은 [[2026-07-29_회고_앞을_얼마나_멀리_볼까요|앞을 얼마나 멀리 볼까요]]에 있어요.
